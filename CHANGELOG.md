...
- **README demo asset** (closes the last item of #20). `scripts/demo.sh` runs a curated two-step demo (quick consult + `--diff` review of a tiny buggy patch); `scripts/record-demo.sh` wraps it under `asciinema rec` and renders an animated `assets/demo.svg` via `svg-term-cli` that GitHub embeds inline. README links the SVG below the lede so first-time visitors see senate running before they read.
